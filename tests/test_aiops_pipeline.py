...
def test_all_anomaly_reasons_are_reported():
    detector = AnomalyDetector()
...
def test_pipeline_command_line_entry_point(tmp_path, monkeypatch, capsys):
    data_directory = tmp_path / "data"
    data_directory.mkdir()
    (data_directory / "service_data.json").write_text("[]", encoding="utf-8")
